...
| **`frames2/*.png`（若存在）**                                                    | **深度模式**下同步导出逐帧 PNG 灰度图（8 位补零命名，如 `00000000.png`），用于对齐样例目录与逐帧可视化。                                                                                                                                                                                            |
...
| **JSONL：`sensor.type: magnetometer` 的 `values`**                               | `CMMagnetometerData.magneticField`（**μT**，与 Spectacular DATA_FORMAT 一致）；与陀螺/加速度共享同一**绝对时间轴**（`CACurrentMediaTime()` 单调时钟秒）；采样率约 **60 Hz**（与设备运动回调独立）。                                                                                                 |
| **JSONL：帧行的 `time`**                                                         | 由首帧 `CACurrentMediaTime()` 绝对时间基准 + `presentationTimeStamp` 增量换算得到，写入与 IMU 一致的**绝对时间轴秒值**。                                                                                                                                                                            |
...
| 字段/行为                    | 说明                                                                                                                                                                                                                             |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **IMU / 磁力计行的 `time`**  | 直接使用 `CACurrentMediaTime()` 作为根级绝对时间秒；与帧行共享同一单调时钟时间轴。                                                                                                                                               |
...
| **`calibration.json`**       | **单路**：`imuToCamera` 使用拍摄期坐标系约定矩阵。**双路**：`cameras` 两项，分辨率与焦距主点对应广角与第二路；深度模式下第二路 `imuToCamera` 优先融合 `AVCameraCalibrationData.extrinsicMatrix` 组合估计，无深度时复制主路估计。 |
...
| **`calibration.json`：`imuToCamera`** | 主路为拍摄期坐标系约定矩阵；深度模式第二路优先基于 `extrinsicMatrix` 组合估计。该值可用于“拍摄可得”效果，但仍非手眼标定或出厂真值。 |
...
| **偏「工程对齐」**     | 绝对时间轴统一、JSONL 排序与缓冲、对焦曝光锁定、深度同步器或 MultiCam PTS 配对。 |
| **偏「占位或未接入」** | `imuToCamera` 真值、畸变真值、原始深度文件。                                     |
