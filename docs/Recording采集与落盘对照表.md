...
| 视频帧 `presentationTimeStamp`     | `data.jsonl`：`frames` 行的根级 `time`（绝对时间轴秒值：首帧 `CACurrentMediaTime` + PTS 增量）                                        |
...
| IMU 回调时刻（可与媒体时钟对齐）   | `data.jsonl`：传感器行的 `time`（`CACurrentMediaTime()` 绝对时间秒）                                                                  |
